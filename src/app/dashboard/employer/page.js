        ...
        <div className="container-fluid card-body">
          <div className="row justify-content-between align-items-center">
            ...
            <div className="col-md-auto mb-3 mb-md-0">
              ...
              <p className="card-text text-muted mb-2">{user?.profile?.designation}</p>
            ...
            <div className={`col-md-auto ${styles.profile}`}>
              <div className="d-flex flex-column align-items-center">
                <Image 
                  width={80} 
                  height={80} 
                  src={user?.profile?.profile_image || '/dashboard/employer/default_company.svg'} 
                  alt={user?.profile?.company_name} 
                  className="rounded-circle object-fit-cover border"
                  style={{ width: '80px', height: '80px' }}
                />
                <div className="mt-2 text-center">
                  <p className="mb-0 fw-medium">{user?.profile?.company_name}</p>
                  <small className="text-muted">
                    <FiMapPin size={14} className="me-1" />
                    {(user?.profile?.company_address)?.substring(0, 10)}...
                  </small>
                </div>
              </div>
            </div>
